...
function CardItem({ imgURL, title, children, isVisited, description, cityID }) {
  ...
  const handleImageLoad = () => {
    setImageLoaded(true);
    setTimeout(() => {
      setLoading(false); // Nasconde il loader solo quando l'immagine è caricata
    }, 3000);
  };

  return (
    <div className={`max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 transition-opacity duration-1000 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: `${cityID * 300}ms` }}>
